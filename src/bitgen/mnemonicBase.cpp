...
std::string MnemonicBase::mnemonicAt(const int num)
{
	return wordList[num];
}
...
int MnemonicBase::numberWords() const
{
	if(mnemonic.empty())
	{
		return 0;
	...
	{
		const unsigned char c = mnemonic[i];
...
//Todo: Move to class MnemonicBase
std::string Mnemonic::bigIntToMnemonic(BigInt<264> total, const int numWords)
{
	std::string ret;
...
//Todo: Move to class MnemonicBase
//Returns the mnemonic as a big integer
BigInt<264> Mnemonic::mnemonicToBigInt(const std::string& mnemonic)
{
	std::string dummy;
	...
	{
		std::cout << "Correct part: \"" << dummy << "\"" << std::endl;
		throw std::string("Mnemonic error");
	}
	
	//std::cout << "Mnemonic words OK: " << dummy << std::endl;
		...
		++it)
	{
		...
		std::map<std::string, int>::const_iterator findIt = wordMap.find(word);
		if(findIt == wordMap.end())
		{
			std::cout << "Can not find word in dictionary: " << word << std::endl;
			throw std::string("The word does not exist");
		}
		
		const int val = findIt->second;
...
std::string MnemonicBase::getFromRandomWithoutChecksum(const BigInt<256>& randomNum, const int numWords)
{
	std::string total;
	bool first(true);
	for(int i = 0 ; i < numWords ; i++)
	{
		const BigInt<256> randomShifted = randomNum >> (11 * i);

		//Take 11 bits to get words, 2^11 = 2048
		const BigInt<256> num = randomShifted.returnMaskHighBits(11);
		
		const std::string word = MnemonicBase::mnemonicAt(num.intVal());	
